...
while True:
    s = input('>')  # 문자열 받기
    if s == '':  # s에 아무것도 없다면
        if Temp == '':  # Temp에도 아무것도 없다면
            print("입력된 텍스트가 없습니다. 다시 입력하여 주세요.")  # 입력한게 없으면 이 텍스트를 띄움
        else:  # 아니면
            break  # 루프문 뿌셔뿌셔 ㅠㅠㅠㅠㅠㅠ
    else:  # 아니면
        Temp = Temp + copy.copy(s)  # 복사하고 다시 돌아가셈 ㅅㄱㅂ
    ...
    if ' ' in a:  # 만약 공백이 있으면
        ...
        try:  # 오류 체크
            ...
            if os.path.isfile(os.path.join(os.getcwd(), "result", f"{name}.wav")):  # 파일이 있으면
                ...
                subprocess.run(
                    f'curl -o {os.path.join(os.getcwd(), "result", f"{name}.mp3")} ' + f'"https://www.google.com/speech-api/v1/synthesize?lang={lang}&speed=0.4&text={a[b]}"')  # name에 저장되있는 문자열을 파일 이름으로 이용, a[b]를 lang에 저장되있는 나라로 음성을 다운
            ...
            break  # 루프를 끝냄 for i in *.wav; do ffmpeg -i "$i" -acodec copy -ss 1 "$i".mp3; done
